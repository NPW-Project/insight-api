...
BlockController.prototype.transformBlock = function(block, info) {
  ...
  return {
    ...
    merkleroot: block.header.merkleRoot.reverse().toString('hex'),
    tx: transactionIds,
    time: block.header.timestamp,
    nonce: block.header.nonce,
    bits: block.header.bits,
    accumulatorcheckpoint: block.header.accumulatorCheckpoint.reverse().toString('hex'),
...
BlockController.prototype.getBlockReward = function(block) {
  if (block.isProofOfWork()) {
    ...
    return npwcore.Unit.fromSatoshis(amt).toNPW();
  } else {
    var tx = block.transactions[1];
    this._transaction.getDetailedTransaction(tx.id, {}, function(err, transaction) {
      return -(transaction.feeSatoshis / 1e8);
    });
  }
};
