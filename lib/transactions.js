...
// transaction parameter is a npwcore transaction
TxController.prototype.transformTransaction = function(transaction, options, callback) {
  ...
  if(transaction.isCoinBase) {
    ...
    transformed.reward = transaction.rewardSatoshis / 1e8;
    transformed.vin = [
      {
        coinbase: transaction.inputs[0].script,
        sequence: transaction.inputs[0].sequenceNumber,
        n: 0
      }
    ];
  } else {
    if (transaction.isCoinStake) {
      ...
      transformed.reward = transaction.rewardSatoshis / 1e8;
      transformed.mnReward = transaction.mnRewardSatoshis / 1e8;
      transformed.posReward = transaction.posRewardSatoshis / 1e8;
    }

    options.inputValues = transaction.__inputValues;
    options.inputScripts = transaction.__inputScripts;
  ...
  transformed.size = npwcore.Transaction(transaction).toBuffer().length;
...
TxController.prototype.transformInput = function(options, input, index) {
  ...
  if (!options.noScriptSig) {
    ...
    if (!options.noAsm) {
      transformed.scriptSig.asm = npwcore.Script(input.script).toASM();
    }
  }

  var address = npwcore.Script(options.inputScripts[index]).toAddress(this._network);
  if (address) {
...
TxController.prototype.transformOutput = function(options, output, index) {
  ...
  if (!options.noAsm) {
    transformed.scriptPubKey.asm = npwcore.Script(output.script).toASM();
  }
  ...
  var address = npwcore.Transaction.Output(output).toAddress(this._network);
  if (address) {
...
TxController.prototype.rawTransaction = function(req, res, next) {
  ...
  this._transaction.getTransaction(txid, function(err, transaction) {
    ...
    req.rawTransaction = {
      'rawtx': npwcore.Transaction(transaction).toString()
    };
